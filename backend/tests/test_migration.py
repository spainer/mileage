...
def test_initial_migration_creates_three_tables(db_url: str) -> None:
    from src.database import configure, run_alembic_upgrade, sync_url
...
def test_deleting_car_cascades_to_children(db_url: str) -> None:
    ...
    session.add(models.MileageRecord(car_id=car.id, date=day, odometer_reading=100))
    session.add(
        models.InsuranceReport(
            car_id=car.id, date=day, odometer_reading=100, mileage_per_year=15000
            )
            )
    session.commit()
...
def test_second_startup_is_noop(db_url: str) -> None:
    ...
    before = set(inspect(create_engine(sync_url(db_url))).get_table_names())

    run_alembic_upgrade(db_url)

    after = set(inspect(create_engine(sync_url(db_url))).get_table_names())
    assert before == after
...
def test_partial_migration_is_logged_not_fatal(
    ...
    from src.database import run_alembic_upgrade, sync_url
